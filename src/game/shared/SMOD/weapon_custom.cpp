...
LINK_ENTITY_TO_CLASS( weapon_custom1, CWeaponCustom );
...
void CWeaponCustom::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, Vector &vecShootOrigin, Vector &vecShootDir )
{
	...
	pOperator->FireBullets( 1, vecShootOrigin, vecShootDir, VECTOR_CONE_PRECALCULATED,
		...
		info.m_iDamage = this->GetWpnData().m_sPrimaryDamage;
...
void CWeaponCustom::ShootBullets( bool isPrimary, bool usePrimaryAmmo )
{
	...
	else
	{
		...
		info.m_iDamage = this->GetWpnData().m_sSecondaryDamage;
...
/*	Vector vecOrigin;
	...
	CBaseEntity *pMissile = CBaseEntity::Create( "rpg_missile", muzzlePoint, vecAngles, CBaseEntity::Instance( GetOwner()->edict() ) );
	pMissile->SetOwnerEntity( Instance( GetOwner()->edict() ) );
	pMissile->Spawn();
