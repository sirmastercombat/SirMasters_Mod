...
//-----------------------------------------------------------------------------
Disposition_t CNPC_AlienGrunt::IRelationType( CBaseEntity *pTarget )
{
	...
	case TASK_ALIENGRUNT_CHARGE:
		{
			...
			if ( AutoMovement( GetEnemy(), &moveTrace ) == false )
			{
				...
				else if ( moveTrace.pObstruction )
				{
					...
					//int disposition = CBaseCombatCharacter::GetDefaultRelationshipDispositionBetweenClasses( CLASS_ANTLION/*CLASS_ALIENGRUNT*/, moveTrace.pObstruction->Classify() );
					if ( IRelationType( moveTrace.pObstruction ) == D_HT )
					{
						// Crash unless we're trying to stop already
						if ( eActivity == ACT_ALIENGRUNT_CHARGE_LOOP )
						{
					...
					else if ( IRelationType( moveTrace.pObstruction ) == D_LI ) 
					{
						// Crash unless we're trying to stop already
						if ( eActivity == ACT_ALIENGRUNT_CHARGE_LOOP )
						{
...
{
	return	CLASS_ANTLION/*CLASS_ALIENGRUNT*/;
}
...
int CNPC_AlienGrunt::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	if (GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE_MAIN_FRAME)
	{
		flMax = 160.0f;
	}*/
	...
	/*if (tr.m_pEnt->Classify() == CLASS_COMBINE_MAIN_FRAME )
	{
...
void CNPC_AlienGrunt::OpenPropDoorNow( CBasePropDoor *pDoor )
{
	...
	if ( (m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT )&& pDoor->HasSpawnFlags(SF_BREAKABLE_BY_NPCS))
	{
		m_hDoor = pDoor;
	...
	case SCHED_FAIL_ESTABLISH_LINE_OF_FIRE:
		{
			...
			/*if (m_hCombineMainFrame)
			{
				return SCHED_TALKER_DESTROY_COMBINE_INTERFACE;
			}*/
...
int	CNPC_AlienGrunt::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
	if ( IsPathTaskFailure( taskFailCode ) &&
		 m_hObstructor != NULL && FClassnameIs( m_hObstructor, "prop_door_rotating") && m_hObstructor->HasSpawnFlags(SF_BREAKABLE_BY_NPCS))
	{
...
void CNPC_AlienGrunt::GatherConditions( void )
{
	...
	{	
		...
		while ( pSquadmate && !bFound)
		{
			if ( pSquadmate != this )
			{
				if ( pSquadmate->Classify() == CLASS_ANTLION/*CLASS_ALIENGRUNT*/ && 
...
void CNPC_AlienGrunt::ImpactShock( const Vector &origin, float radius, float magnitude, CBaseEntity *pIgnored )
{
	...
	while ( ( pEntity = gEntList.FindEntityInSphere( pEntity, origin, radius ) ) != NULL )
	{
		...
		if (pDoor)
		{
			if (pDoor->HasSpawnFlags(SF_BREAKABLE_BY_NPCS) && (WorldSpaceCenter() - pDoor->WorldSpaceCenter()).Length() < radius )
			{
				// add some spin so the object doesn't appear to just fly in a straight line
				// Also this spin will move the object slightly as it will press on whatever the object
				// is resting on.
